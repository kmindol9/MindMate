...
|---------------|-----------|-------------|--------|
|  2026-05-08   |  2.0.0    | 분석 문서 작성 완료 | 김민석 |
|  2026-06-12   |  2.0.1    | 구현 이후 분석 문서 재작성   | 김민석 |
|               |           |             |        |
...
#### Use Case #3 : Classify Problem

- **Summary:** 감정 분석 결과와 입력 내용을 바탕으로 사용자의 문제 유형을 분류하는 기능
- **Primary Actor:** System
- **Preconditions:** 감정 분석 결과 생성 완료
- **Main Success Scenario:**
  1. 시스템은 입력 내용과 감정 분석 결과를 바탕으로 문제 유형을 판단한다.
  2. 문제 유형을 진로 고민, 학업 스트레스, 인간관계 문제, 비교 불안, 소비 및 금전 고민, 일상 선택 고민, 시간 관리 문제, 미래 불안 등으로 분류한다.
  3. 분류된 문제 유형은 데이터 조회 및 피드백 생성 단계에서 기준 정보로 사용된다.
  
---

#### Use Case #4 : Structure Problem

- **Summary:** 사용자의 고민을 감정, 원인, 상황, 대안 중심으로 구조화하는 기능
- **Primary Actor:** System
- **Preconditions:** 문제 유형 분류 완료
- **Main Success Scenario:**
1. 시스템은 사용자의 고민에서 핵심 원인을 추출한다.
2. 복잡한 고민을 감정 상태, 문제 유형, 핵심 원인, 객관적 관점으로 나누어 정리한다.
3. 사용자가 자신의 상황을 더 명확하게 이해할 수 있도록 구조화된 분석 결과를 생성한다.

---

#### Use Case #5 : Retrieve Data
- **Summary:** 문제 유형에 맞는 통계 및 사례 데이터를 조회하는 기능
- **Primary Actor:** System, Data Source
- **Preconditions:** 문제 유형 분류 완료
- **Main Success Scenario:**
1. 시스템은 분류된 문제 유형을 기준으로 관련 데이터를 조회한다.
2. 문제 유형에 맞는 통계, 사례, 참고 근거를 수집한다.
3. 조회된 데이터는 최종 피드백 생성 단계에서 객관적인 근거로 활용된다.

---

#### Use Case #6 : Generate Feedback
- **Summary:** 분석 결과와 근거 데이터를 기반으로 현실적인 대안과 방향성을 제공하는 기능
- **Primary Actor:** System, AI Module
- **Preconditions:** 감정 분석, 문제 분류, 문제 구조화, 데이터 조회 완료
- **Main Success Scenario:**
1. 시스템은 감정 분석 결과, 문제 유형, 핵심 원인, 관련 데이터를 통합한다.
2. 사용자의 의사결정을 대신하지 않고, 현실적인 선택지와 판단 기준을 제시한다.
3. 외부 AI API가 사용 가능한 경우 자연어 기반 피드백을 생성한다.
4. 생성된 피드백은 최종 결과 화면에 전달된다.

---

#### Use Case #7 : Display Result
- **Summary:** 최종 분석 결과와 피드백을 사용자에게 제공하는 기능
- **Primary Actor:** System
- **Preconditions:** 피드백 생성 완료
- **Main Success Scenario:**
1. 시스템은 분석 결과를 감정 분석, 문제 유형, 핵심 원인, 객관적 관점, 현실적인 대안, 관련 데이터 및 근거로 구분한다.
2. 각 항목을 사용자가 이해하기 쉬운 카드 형태로 화면에 출력한다.
3. 사용자는 최종 분석 결과를 확인하고 자신의 상황을 객관적으로 정리할 수 있다.

---

#### Use Case #8 : Select Decision Support Mode
- **Summary:** 사용자가 간단한 선택 상황을 분석하기 위해 의사결정 지원 모드를 선택하는 기능
- **Primary Actor:** User
- **Preconditions:** 사용자가 MindMate 웹 서비스에 접속한 상태
- **Main Success Scenario:**
1. 사용자는 상단 네비게이션에서 간단 의사결정 메뉴를 선택한다.
2. 시스템은 의사결정 지원 입력 화면을 제공한다.
3. 사용자는 일상적인 선택 상황이나 고민을 입력한다.
4. 시스템은 입력 내용을 장단점, 판단 기준, 현실적인 선택 방향 중심으로 분석한다.

---

#### Use Case #9 : Use Fallback Mode
- **Summary:** 외부 AI API가 응답하지 못하는 상황에서 내부 규칙 기반 분석 결과를 제공하는 기능
- **Primary Actor:** System
- **Preconditions:** 외부 AI API 요청 실패, 요청 한도 초과, 네트워크 오류 또는 서버 오류 발생
- **Main Success Scenario:**
1. 시스템은 외부 AI API 호출 실패를 감지한다.
2. 예외 처리를 통해 서비스 중단을 방지한다.
3. 내부 키워드 기반 규칙을 사용하여 문제 유형을 분류한다.
4. 기본 감정 분석, 핵심 원인, 객관적 관점, 현실적인 대안을 생성한다.
5. 사용자에게 기본 분석 모드 결과를 제공한다.
...
| --------------------- | ------------------------------ | ------------------------------------ |
| **User**              | 시스템 사용자 및 고민 입력의 주체            | 
`concernText`, `emotionState`        |
| **InputHandler**      | 사용자의 입력을 수집하고 전처리하는 클래스        | 
`inputText`, `processedText`         |
| **EmotionAnalyzer**   | 입력된 텍스트를 기반으로 감정을 분석하는 클래스     | 
`emotionType`, `aiResponse`     |
| **ProblemClassifier** | 감정 분석 결과를 바탕으로 문제 유형을 분류하는 클래스 | `problemCategory`, `problemLevel`    |
| **ProblemStructurer** | 분석 결과를 감정, 문제 유형, 핵심 원인, 객관적 관점, 대안으로 구조화하는 클래스 | `coreCause`, `objectiveView`, `alternatives` |
| **DataRetriever**     | 통계 및 사례 데이터를 조회하는 클래스          |
`statisticsData`, `caseData` |
| **FeedbackGenerator** | 최종 피드백과 대안을 생성하는 클래스           | 
`feedbackText`, `alternativeOptions` |
| **ResultViewer** | 최종 분석 결과를 사용자에게 카드 형태로 출력하는 클래스 |
`resultText`, `resultCards` |
